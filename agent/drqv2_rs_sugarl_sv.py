...
if __name__ == "__main__":
    ...
    drq_agent = DrQV2SugarlAgent(
        (envs.single_observation_space["active_view_image"].shape[0]*args.pvm_stack, *envs.single_observation_space["active_view_image"].shape[1:]),
    ...
    rb = NstepRewardDoubleActionReplayBuffer(
        ...
        observation_space=Box(low=-1, high=1., shape=(envs.single_observation_space["active_view_image"].shape[0]*args.pvm_stack, *envs.single_observation_space["active_view_image"].shape[1:])),
    ...
    pvm_buffer = PVMBuffer(args.pvm_stack, (envs.num_envs, *envs.single_observation_space["active_view_image"].shape))
    reset_pvm_buffer = False
    while global_transitions < args.total_timesteps:
        pvm_buffer.append(obs)
        pvm_obs = pvm_buffer.get_obs(mode="stack_channel")
        ...
        real_next_pvm_obs = pvm_buffer_copy.get_obs(mode="stack_channel")
        rb.add(pvm_obs, real_next_pvm_obs, motor_actions, sensory_actions, rewards, dones, {"selected_cam": selected_cam})
        ...
        if (global_transitions % args.eval_frequency == 0 and args.eval_frequency > 0) or \
            ...
            for eval_ep in range(args.eval_num):
                ...
                eval_pvm_buffer = PVMBuffer(args.pvm_stack, (envs.num_envs, *envs.single_observation_space["active_view_image"].shape))
                done = False
                while not done:
                    eval_pvm_buffer.append(eval_obs)
                    eval_pvm_obs = eval_pvm_buffer.get_obs(mode="stack_channel")
                    ...
                    eval_next_obs, rewards, dones, _, infos = eval_env.step(
                        {"motor_action": processed_actions["motor_action"],
                        "sensory_action": processed_actions["sensory_action"]}
                    )
                    ...
                    if done:
                        ...
                        if args.capture_video:
                            ...
                            if eval_ep == 0:
                                ...
                                model_fn = f"{args.task_name}_seed{args.seed}_{global_transitions:07d}_model.pt"
