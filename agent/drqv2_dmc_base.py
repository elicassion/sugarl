...
from active_gym import DMCBaseEnv, DMCEnvArgs
...
class DrQV2Agent:
    ...
    def act(self, obs, step, eval_mode=None) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    while global_transitions < args.total_timesteps:
        actions = drq_agent.act(torch.Tensor(obs).to(device), global_transitions)
        ...
        if (global_transitions % args.eval_frequency == 0 and args.eval_frequency > 0) or \
            ...
            for eval_ep in range(args.eval_num):
                ...
                while not done:
                    actions = drq_agent.act(torch.Tensor(eval_obs).to(device), step=global_transitions)
                    eval_next_obs, rewards, dones, _, infos = eval_env.step(actions)
