...
class DrQV2ActiveAgent:
    ...
    def update_critic(self, obs, motor_action, sensory_action, reward, discount, next_obs, step):
        ...
        with torch.no_grad():
            ...
            motor_target_Q = reward + (discount * motor_target_V)
            sensory_target_V = torch.min(sensory_target_Q1, sensory_target_Q2)
            sensory_target_Q = reward + (discount * sensory_target_V)

        motor_q1, motor_q2, sensory_q1, sensory_q2 = self.critic(obs, motor_action, sensory_action)
        physical_critic_loss = F.mse_loss(motor_q1, motor_target_Q) + \
                        F.mse_loss(motor_q2, motor_target_Q)
        visual_critic_loss = F.mse_loss(sensory_q1, sensory_target_Q) + \
                        F.mse_loss(sensory_q2, sensory_target_Q)
        
        critic_loss = physical_critic_loss + visual_critic_loss
...
if __name__ == "__main__":
    ...
    while global_transitions < args.total_timesteps:
        motor_actions, sensory_actions, processed_actions = drq_agent.act(torch.Tensor(obs).to(device), global_transitions)
        ...
        rb.add(obs, real_next_obs, motor_actions, sensory_actions, rewards, dones, [{}])
        ...
        if (global_transitions % args.eval_frequency == 0 and args.eval_frequency > 0) or \
            ...
            for eval_ep in range(args.eval_num):
                ...
                while not done:
                    motor_actions, sensory_actions, processed_actionss = drq_agent.act(torch.Tensor(eval_obs).to(device), step=global_transitions)
